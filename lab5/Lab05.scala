...
object Lab05 extends lisa.Main {
  ...
  addSymbol(<=)
  val u = ConstantFunctionLabel(
    "u",
    2
  ) // join (union for sets, disjunction in boolean algebras)
  addSymbol(u)
  val n = ConstantFunctionLabel(
    "n",
    2
  ) // meet (interestion for sets, conjunction in boolean algebras)
  addSymbol(n)
  ...
  val joinLowerBound = Theorem((x <= (x u y)) /\ (y <= (x u y))) {
    have(thesis) by Tautology.from(
      lub of (z := (x u y)),
      reflexivity of (x := (x u y))
    )
  }

  val joinCommutative = Theorem((x u y) === (y u x)) {
    val s1 = have((x u y) <= (y u x)) by Tautology.from(
      lub of (z := (y u x)),
      joinLowerBound of (x := y, y := x)
    )
    have(thesis) by Tautology.from(
      s1,
      s1 of (x := y, y := x),
      antisymmetry of (x := x u y, y := y u x)
    )
  }
  ...
  val joinAssociative = Theorem((x u (y u z)) === ((x u y) u z)) {
    val left = have((x u (y u z)) <= ((x u y) u z)) subproof {
      val a1 = have(x <= ((x u y) u z)) by Tautology.from(
        joinLowerBound,
        joinLowerBound of (x := (x u y), y := z),
        transitivity of (y := (x u y), z := ((x u y) u z))
      )
      val a2 = have(y <= ((x u y) u z)) by Tautology.from(
        joinLowerBound,
        joinLowerBound of (x := (x u y), y := z),
        transitivity of (x := y, y := (x u y), z := ((x u y) u z))
      )
      val a3 = have(z <= ((x u y) u z)) by Tautology.from(
        joinLowerBound of (x := (x u y), y := z)
      )
      have(thesis) by Tautology.from(
        a1,
        ...
        lub of (y := (y u z), z := ((x u y) u z))
      )
    }
    val right = have(((x u y) u z) <= (x u (y u z))) subproof {
      val b1 = have(x <= (x u (y u z))) by Tautology.from(
        joinLowerBound of (y := (y u z))
      )
      val b2 = have(y <= (x u (y u z))) by Tautology.from(
        joinLowerBound of (x := y, y := z),
        joinLowerBound of (y := (y u z)),
        transitivity of (x := y, y := (y u z), z := (x u (y u z)))
      )
      val b3 = have(z <= (x u (y u z))) by Tautology.from(
        joinLowerBound of (x := y, y := z),
        joinLowerBound of (y := (y u z)),
      ...
      have(thesis) by Tautology.from(
        ...
        b3,
        lub of (z := (x u (y u z))),
        lub of (x := (x u y), y := z, z := (x u (y u z)))
      )
    }
    have(thesis) by Tautology.from(
      left,
      right,
      antisymmetry of (x := (x u (y u z)), y := ((x u y) u z))
    )
  }
  ...
  object Whitman extends ProofTactic {
    def solve(using lib: library.type, proof: lib.Proof)(
        goal: Sequent
    ): proof.ProofTacticJudgement = {
      if goal.left.nonEmpty || goal.right.size != 1 then
        proof.InvalidProofTactic(
          "Whitman can only be applied to solve goals of the form () |- s <= t"
        )
      else
