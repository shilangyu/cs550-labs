...
object Mansion {
  ...
  def leibnizProp(predicate: Term => Predicate): Formula = Forall(
    nvar("x"),
    Forall(
      nvar("y"),
      Implies(
        eqvp(nvar("x"), nvar("y")),
        Implies(predicate(nvar("x")), predicate(nvar("y")))
      )
    )
  )
    ...
    List(
      ...
      and(
        List(
          ...
          Forall(
            nvar("x"),
            Implies(livesp(x), or(List(eqvp(x, a), eqvp(x, b), eqvp(x, c))))
          )
        )
      ),
      Forall(
        ...
        Forall(
          nvar("y"),
          Implies(killedp(x, y), and(List(hatesp(x, y), Neg(richerp(x, y)))))
        )
      ),
  ...
  val additionalAssumptions: Formula = and(
    List(
      ...
      forall(
        ...
        forall(
          "y",
          Implies(eqvp(nvar("x"), nvar("y")), eqvp(nvar("y"), nvar("x")))
        )
      ),
  ...
  val baseFormula = conjunctionPrenexSkolemizationNegation(
    And(mansionMystery, additionalAssumptions)
  )
  ...
  def buildFullProof(
      missing1: ResolutionProof,
      missing2: BigInt => ResolutionProof
  ) = {
    ...
    val prelude = firstPart ++ List(
      ...
      (
        List(eqv(killer, svar(16)).negation, killed(svar(16), a)),
        Deduced((1, 17), Map(id(14) -> a, id(15) -> killer))
      ),
      // -10) Charles isn't the killer
      (
        List(eqv(killer, c).negation),
      ...
      (
        List(hates(a, notHatedBy(b)).negation),
      ...
      (
        List(hates(b, b).negation, hates(b, notHatedBy(b))),
        Deduced(
          (19, offset + 5),
          Map(id(20) -> b, id(21) -> b, id(22) -> notHatedBy(b))
        )
      ),
