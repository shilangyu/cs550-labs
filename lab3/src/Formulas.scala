...
object Formulas {
  ...
  sealed abstract class Term {
    ...
    def substitute(subst: Map[Identifier, Term]): Term = {
      this match {
        ...
        case Function(name, children) =>
          Function(name, children.map(_.substitute(subst)))
      }
    }
    ...
    override def toString(): String = this match {
      ...
      case Function(id, children) =>
        s"${id}(${mkString(children.map(_.toString()))})"
    }
  }
  ...
  /** A "box" for literals. One may wonder "why are we boxing formulas into this
    * case class?". If we did not do that, ensuring that a `List[List[Formula]]`
    * (which is how we represent formulas in CNF) is indeed in CNF would mean
    * stating the following: cnf.forall(clause => clause.forall(literal =>
    * literal.isLiteral)) which is both verbose and impractical, one would need
    * to use many lemmas on lists to ensure e.g. cnf(0)(1) // "The second
    * literal of the first clause is indeed a literal"
    *
    * It turns out to be much simpler to box formulas satisfying this condition
    * into this class, which then ensures implicitly that the formulas are
    * literals.
    */
  ...
  /** You can safely ignore the [[@targetName]] annotations. See
